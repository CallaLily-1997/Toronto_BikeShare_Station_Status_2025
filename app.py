...
st.markdown("This dashboard tracks bite availability at each bike share station in Toronto")
...
with st.sidebar:
    ...
    if bike_method == "Rent":
        st.markdown("""
        🟢 **Green**: Many bikes available (>3)  
        🟡 **Yellow**: Few bikes available (1–3)  
        🔴 **Red**: No bikes available  
        """)
    else:
        st.markdown("""
        🟢 **Green**: Many empty docks available  
        🟡 **Yellow**: Few empty docks available  
        🔴 **Red**: No empty docks available  
        """)
